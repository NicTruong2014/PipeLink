...
#define ZOOMIN_ZOOMOUT_ACTION_TAG 3322
...
bool AnimationManager::init()
{
  ...
  /*  auto animation = AnimationCache::getInstance()->getAnimation("animationWater");

    if (animation == nullptr)
    {
        animation = AnimationManager::GetInstance()->createAnimation("water/water_000%02d.png", 1, 37, 0.05,1);

        AnimationCache::getInstance()->addAnimation(animation, "animationWater");
    }*/
	return true;
}
...
void AnimationManager::zoomInZoomOutEffectForever(Node* i_node, float duration, float s)
{
	...
	i_node->runAction(RepeatForever::create(Sequence::create(ScaleTo::create(k_duration, k_currentScaleFactor + 0.1f + s), ScaleTo::create(k_duration, k_currentScaleFactor), ScaleTo::create(k_duration, k_currentScaleFactor + 0.1f + s), ScaleTo::create(k_duration, k_currentScaleFactor), nullptr)));
}

void AnimationManager::zoomInZoomOutEffect(Node * i_node, std::function<void()> i_callback, float duration, float s)
{
	MK_RETURN_IF_WITH_CONTENT(i_node == nullptr, , if (i_callback) i_callback(););
	...
	i_node->stopActionByTag(ZOOMIN_ZOOMOUT_ACTION_TAG);

	auto action = Sequence::create(ScaleTo::create(k_duration, k_currentScaleFactor + 0.1f + s), ScaleTo::create(k_duration, k_currentScaleFactor), ScaleTo::create(k_duration, k_currentScaleFactor + 0.1f + s), ScaleTo::create(k_duration, k_currentScaleFactor), CallFunc::create([i_callback]() {
		if (i_callback) {
			i_callback();
		}
	}), nullptr);

	action->setTag(ZOOMIN_ZOOMOUT_ACTION_TAG);

	i_node->runAction(action);
}
...
void AnimationManager::tintAnimation(cocos2d::Node* node, float actionTime, std::function<void()> callback)
{
	auto actionTint = TintBy::create(actionTime, -255, -127, 0);
	auto actionTintBack = actionTint->reverse();
	auto action = Sequence::create(actionTint, actionTintBack, nullptr);
	node->runAction(RepeatForever::create(action));
}

void AnimationManager::tintAnimation(cocos2d::Node* node, float actionTime, int actionTag, Color3B color)
{
	if (!node) { return; }

	node->stopAllActionsByTag(actionTag);

	auto actionTint = TintBy::create(actionTime, color.r, color.g, color.b);
	auto actionTintBack = actionTint->reverse();
	auto action = RepeatForever::create(Sequence::createWithTwoActions(actionTint, actionTintBack));
...
Animation* AnimationManager::createAnimation(const char* prefixName, int start, int end, float delay, int loop )
{
	...
	for (int i = start; i <= end; i++)
	{
        ...
        {
			auto sprite = Sprite::createWithSpriteFrameName(String::createWithFormat(prefixName, i)->getCString());
...
void AnimationManager::runShakingAction(cocos2d::Node* node, int actionTag)
{
	if (!node) {
		return;
	}
...
void AnimationManager::runShakingAction(cocos2d::Node* node, float duration, int actionTag)
{
    if (!node) {
        return;
    }

    float t = duration/7.0f;
    ...
    if (!shaking) {
        ...
        auto shakingAction = Sequence::create(
            MoveBy::create(t, { 10,10 }),
            MoveBy::create(t, { -10,-10 }),
            MoveBy::create(t, { -10,-10 }),
            MoveBy::create(t, { +10,+10 }),
            MoveBy::create(t, { -10,0 }),
            MoveBy::create(t, { +20,0 }),
            MoveBy::create(t, { -10,0 }),
            nullptr);
